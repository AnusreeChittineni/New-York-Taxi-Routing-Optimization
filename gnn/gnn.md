...
This document explains the GNN used in the NYC Traffic Simulation project — its architecture, learning objective, and training policy.  
The model predicts **edge-level travel times** from **node and edge features** of the Manhattan road network.  
It serves as the foundation for estimating travel times under normal conditions and evaluating **road closure impacts**.
...
The GNN is not trained on labeled paths — only **start/end** locations and **observed travel durations**.  
We simulate the latent route discovery process through **candidate path generation** and **policy-based exploration**.
...
2. **Weighted Average Travel Time (WAvgTT)**
   \[
   \text{WAvgTT} = \mathbb{E}_{(o,d)\sim q_\text{hotspot}}[\min_j \sum_{e\in p_{ij}}\hat{t}(e)]
   \]
...
### ✨ **Summary**
This GNN combines **spatial reasoning**, **probabilistic routing**, and **traffic equilibrium modeling** into one unified learning system.  
It learns from trip-level observations alone — discovering both **realistic routes** and **edge-level congestion patterns** — enabling simulations of **road closures**, **traffic rerouting**, and **network resilience**.
